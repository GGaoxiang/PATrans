import os
import numpy as np
from patrans_core.structures.boxlist_ops import boxlist_iou
...
#gt_box: x1,y1,x2,y2
def polyp_center_distance(det_box, gt_box):

    center_x_det = float(det_box[0] + (det_box[2]-det_box[0])/2)
    center_y_det = float(det_box[1] + (det_box[3]-det_box[1])/2)
    center_x_gt = float(gt_box[0] + (gt_box[2]-gt_box[0])/2)
    center_y_gt = float(gt_box[1] + (gt_box[3]-gt_box[1])/2)

    distance = np.sqrt((center_x_det-center_x_gt)**2 + (center_y_det-center_y_gt)**2)
...
def cvcvideo_localization_center_eval(pred_boxlists, gt_boxlists, score_thrs):
    ...
    detection_fp = np.zeros((len(pred_boxlists), score_thrs.shape[0]), dtype=np.int)
    detection_fn = np.zeros((len(pred_boxlists), score_thrs.shape[0]), dtype=np.int)
    distance = np.zeros((len(pred_boxlists), score_thrs.shape[0]), dtype=np.float)
    gt_num = 0
            ...
            if highscore_bbox.shape[0]>0:
                ...
                for gt_idx in range(gt_label.sum()):
                    if det_for_each_gt[gt_idx] > 0:
                        detection_tp[idx,score_idx] += 1
                        distance[idx,score_idx] = polyp_center_distance(highscore_bbox[center_flag_list[:,gt_idx]][0], gt_bbox[gt_idx])
                    else:
                        detection_fn[idx,score_idx] += 1
    ...
    mean_dist = np.mean(distance, axis=0)

    return np.vstack((Precision, Recall, F1_score, F2_score, mean_dist)), \
