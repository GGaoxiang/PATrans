...
class CVCVIDMEGADataset(CVCVIDImageDataset):
    def __init__(self, image_set, data_dir, img_dir, anno_path, img_index, transforms, is_train=True):
        super(CVCVIDMEGADataset, self).__init__(image_set, data_dir, img_dir, anno_path, img_index, 
        ...
        if not self.is_train:
            ...
            for id, frame_id in enumerate(self.frame_id):
                if frame_id == 1:
                    ...
                    if cfg.MODEL.VID.MEGA.GLOBAL.ENABLE:
                        shuffled_index = np.arange(1, self.frame_seg_len[id]+1)
    ...
    def _get_train(self, idx):
        ...
        if hasattr(self, "pattern"):
            ...
            if cfg.MODEL.VID.MEGA.MEMORY.ENABLE:
                ref_id_center = max(self.frame_seg_id[idx] - cfg.MODEL.VID.MEGA.ALL_FRAME_INTERVAL, 1)
                offsets = np.random.choice(cfg.MODEL.VID.MEGA.MAX_OFFSET - cfg.MODEL.VID.MEGA.MIN_OFFSET + 1,
                                           cfg.MODEL.VID.MEGA.REF_NUM_MEM, replace=False) + cfg.MODEL.VID.MEGA.MIN_OFFSET
                for i in range(len(offsets)):
                    ref_id = min(max(ref_id_center + offsets[i], 1), self.frame_seg_len[idx])
                    ref_filename = self.pattern[idx] % ref_id
                    img_ref = Image.open(self._img_dir % ref_filename).convert("RGB")
            ...
            # global frames
            if cfg.MODEL.VID.MEGA.GLOBAL.ENABLE:
                ref_ids = np.random.choice(self.frame_seg_len[idx], cfg.MODEL.VID.MEGA.REF_NUM_GLOBAL, replace=False) + 1
    ...
    def _get_test(self, idx):
        ...
        ref_id = min(self.frame_seg_len[idx], self.frame_seg_id[idx] + cfg.MODEL.VID.MEGA.MAX_OFFSET)
        ref_filename = self.pattern[idx] % ref_id
        img_ref = Image.open(self._img_dir % ref_filename).convert("RGB")
        img_refs_l.append(img_ref)
        ...
        img_refs_g = []
        if cfg.MODEL.VID.MEGA.GLOBAL.ENABLE:
            size = cfg.MODEL.VID.MEGA.GLOBAL.SIZE if frame_id == 1 else 1
