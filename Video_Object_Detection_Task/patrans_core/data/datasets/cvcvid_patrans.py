...
class CVCVIDPATRANSDataset(CVCVIDImageDataset):
    def __init__(self, image_set, data_dir, img_dir, anno_path, img_index, transforms, is_train=True):
        super(CVCVIDPATRANSDataset, self).__init__(image_set, data_dir, img_dir, anno_path, img_index, 
    ...
    def _get_train(self, idx):
        ...
        if hasattr(self, "pattern"):
            offsets = np.random.choice(-cfg.MODEL.VID.PATRANS.MIN_OFFSET, int(cfg.MODEL.VID.PATRANS.TRAIN_REF_NUM/2), replace=False) + cfg.MODEL.VID.PATRANS.MIN_OFFSET
            ...
            offsets = np.random.choice(cfg.MODEL.VID.PATRANS.MAX_OFFSET, int(cfg.MODEL.VID.PATRANS.TRAIN_REF_NUM/2), replace=False) + 1
            for i in range(len(offsets)):
                ref_id = min(max(self.frame_seg_id[idx] + offsets[i], 1), self.frame_seg_len[idx])
                ref_filename = self.pattern[idx] % ref_id
                img_ref = Image.open(self._img_dir % ref_filename).convert("RGB")
            ...
            for i in range(cfg.MODEL.VID.PATRANS.TRAIN_REF_NUM):
    ...
    def _get_test(self, idx):
        ...
        ref_id = min(self.frame_seg_len[idx], self.frame_seg_id[idx] + cfg.MODEL.VID.PATRANS.MAX_OFFSET)
        ref_filename = self.pattern[idx] % ref_id
        img_ref = Image.open(self._img_dir % ref_filename).convert("RGB")
        img_refs.append(img_ref)
