...
from patrans_core.layers import ModulatedDeformConvPack as DCN
...
def get_center_net(cfg):
    resnet_spec = {18: (BasicBlock, [2, 2, 2, 2]),
                34: (BasicBlock, [3, 4, 6, 3]),
                50: (Bottleneck, [3, 4, 6, 3]),
                101: (Bottleneck, [3, 4, 23, 3]),
                152: (Bottleneck, [3, 8, 36, 3])}
    num_layers = cfg.MODEL.BACKBONE.CONV_BODY
    num_layers = int(num_layers.split('-')[-1])
    block_class, layers = resnet_spec[num_layers]

    heads = {'hm': cfg.MODEL.CENTERNET.CHANNEL_HM,
    ...
    model = ResNetDCN(block_class, layers, heads, head_conv=head_conv)
