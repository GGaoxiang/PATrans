...
from patrans_core.layers import Scale
from patrans_core.layers import DFConv2d
from patrans_core.layers import DeformConv
from .inference import make_fcos_stft_postprocessor
from .loss import make_fcos_stft_loss_evaluator
from patrans_core.PATrans_module.models.patrans import PATrans
...
class PATRANSFCOSHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        """
        Arguments:
            in_channels (int): number of channels of the input feature
        """
        super(PATRANSFCOSHead, self).__init__()
        ...
        self.cls_encoder = nn.Sequential(
            nn.Conv2d(num_classes, in_channels//2, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(in_channels//2, in_channels, kernel_size=3, stride=1, padding=1))
        self.reg_encoder = nn.Sequential(
            nn.Conv2d(4, in_channels//2, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(in_channels//2, in_channels, kernel_size=3, stride=1, padding=1))
        ...
        # Transformer Setting
        self.PREV_IMG_MODE = cfg.MODEL.TRANS.PREV_IMG_MODE
        self.SUP_GT_MODE = cfg.MODEL.TRANS.SUP_GT_MODE
        self.use_transformer_layer = cfg.MODEL.TRANS.TRANS_FEAT_LAYER
        self.VIEW_NUM = cfg.MODEL.TRANS.VIEW_NUM
        self.Self_CosPostion = cfg.MODEL.TRANS.MODEL_SELF_COSPOSITION
        self.PATrans_cls = PATrans(cfg, int(cfg.MODEL.TRANS.MODEL_ENCODER_EMBEDDING_DIM_CLS)) 
        self.PATrans_reg = PATrans(cfg, int(cfg.MODEL.TRANS.MODEL_ENCODER_EMBEDDING_DIM_REG))
    ...
    def forward(self, x, shifts):
        ...
        for l, (feature, shifts_i) in enumerate(zip(x, shifts)):
            ...
            # PATrans -- classification
            stft_cls_feats = self.dcn_cls_subnet(cls_tower, align_ltrb_off)
            # cls
            if l >= self.use_transformer_layer:
                if self.PREV_IMG_MODE == 'before_ST':
                    target_stft_patrans_cls_feats = cls_tower[0] # 1,C,H,W
                    support_stft_patrans_cls_feats = cls_tower[1:] # N-1, C, H, W
                elif self.PREV_IMG_MODE == 'after_ST':
                    target_stft_patrans_cls_feats = stft_cls_feats[0] # 1,C,H,W
                    support_stft_patrans_cls_feats = stft_cls_feats[1:] # N-1, C, H, W
                else:
                    print("wrong PREV_IMG_MODE")
                cls_logits = self.cls_encoder(cls_logits)[1:].permute(2,3,0,1).contiguous().view(H*W, -1, C)
                if self.Self_CosPostion:
                    pos_emb = self.PATrans_cls.get_pos_emb(target_stft_patrans_cls_feats.unsqueeze(0)).expand(1, -1, -1,
                        -1).view(1, -1, H*W).permute(2, 0, 1)  
                    pos_emb_sup = self.PATrans_cls.get_pos_emb(support_stft_patrans_cls_feats).expand(N-1, -1, -1,
                        -1).view(N-1, -1, H*W).permute(2, 0, 1)
                else:
                    pos_emb = None
                    pos_emb_sup = None
                target_stft_patrans_cls_feats = self.PATrans_cls.PATrans_forward(target_stft_patrans_cls_feats,
                                                        support_stft_patrans_cls_feats,
                                                        cls_logits,
                                                        pos_emb=pos_emb,
                                                        pos_emb_sup = pos_emb_sup,
                                                        size_2d=(H,W)).view(H, W, 1, C).permute(2, 3, 0, 1)
                stft_cls_logits = self.dcn_cls_score(target_stft_patrans_cls_feats)
            ...
            if l >= self.use_transformer_layer:
                if self.PREV_IMG_MODE == 'before_ST':
                    target_stft_patrans_reg_feats = box_tower[0] # 1,C,H,W
                    support_stft_patrans_reg_feats = box_tower[1:] # N-1, C, H, W
                elif self.PREV_IMG_MODE == 'after_ST':
                    target_stft_patrans_reg_feats = stft_reg_feats[0] # 1,C,H,W
                    support_stft_patrans_reg_feats = stft_reg_feats[1:] # N-1, C, H, W
                else:
                    print("wrong PREV_IMG_MODE")
                    
                if self.SUP_GT_MODE == 'bbox_pred':
                    bbox_pred = self.reg_encoder(bbox_pred)[1:].permute(2,3,0,1).contiguous().view(H*W, -1, C)
                elif self.SUP_GT_MODE == 'align_ltrb_off':
                    bbox_pred = self.reg_encoder(align_ltrb_off.view(N,H,W,-1).permute(0,3,1,2))[1:].permute(2,3,0,1).contiguous().view(H*W, -1, C)

                if self.Self_CosPostion:
                    pos_emb = self.PATrans_reg.get_pos_emb(target_stft_patrans_reg_feats.unsqueeze(0)).expand(1, -1, -1,
                        -1).view(1, C,H*W).permute(2, 0, 1)  
                    pos_emb_sup = self.PATrans_reg.get_pos_emb(support_stft_patrans_reg_feats).expand(N-1, -1, -1,
                        -1).view(N-1, C, H*W).permute(2, 0, 1)
                else:
                    pos_emb = None
                    pos_emb_sup = None

                target_stft_patrans_reg_feats = self.PATrans_reg.PATrans_forward(target_stft_patrans_reg_feats,
                                                        support_stft_patrans_reg_feats,
                                                        bbox_pred,
                                                        pos_emb=pos_emb,
                                                        pos_emb_sup = pos_emb_sup,
                                                        size_2d=(H,W)).view(H, W, 1, C).permute(2, 3, 0, 1)
                
                stft_bbox_reg_pred = self.dcn_bbox_pred(target_stft_patrans_reg_feats)
...
def build_fcos_patrans(cfg, in_channels):
    return PATRANSFCOSModule(cfg, in_channels)
