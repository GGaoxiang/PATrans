...
def flatten_probas(probas, labels, ignore=255):
    ...
    """
    B, C, H, W = probas.size()
    # print('B,C,H,W:', B, C, H, W, 'ignore:', ignore) 
    #1,2,465,465; 255
    probas = probas.permute(0, 2, 3,
                            1).contiguous().view(-1, C)  # B * H * W, C = P, C
    labels = labels.view(-1)
    # print('probas:', probas.shape, probas.type())
    #[216225, 2], float32, 
    ...
    valid = (labels != ignore)
    # print('valid:', valid.shape, valid.type())
    #[216225], bool
    
    # prob_valid = valid.view(-1, 1).expand(-1, C)
    # print('prob_valid:', prob_valid.shape, prob_valid.type())
    # #prob_valid: torch.Size([216225, 2]) torch.float16
    # probas = probas.half()
    # print('probas:', probas.shape, probas.type())
    # vprobas = probas[prob_valid]
    # print('vprobas:', vprobas.shape, vprobas.type())
    # vprobas = vprobas.reshape(-1, C)
    # print('vprobas:', vprobas.shape, vprobas.type())
...
class CrossEntropyLoss(nn.Module):
    def __init__(self,
                 top_k_percent_pixels=None, #0.15
                 hard_example_mining_step=100000): #0.5*100000
