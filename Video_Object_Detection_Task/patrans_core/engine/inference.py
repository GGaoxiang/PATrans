...
from patrans_core.data.datasets.evaluation import evaluate
from ..utils.comm import is_main_process, get_world_size
from ..utils.comm import all_gather
from ..utils.comm import synchronize
from ..utils.timer import Timer, get_time_str
from .bbox_aug import im_detect_bbox_aug
from patrans_core.structures.bounding_box import BoxList
    ...
    for i, batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            if bbox_aug:
                ...
                elif method in ("rdn", "mega", "fgfa", "stft", "cvc_fgfa", "cvc_mega", "cvc_rdn", "cvc_stft", "cvc_patrans"):
...
def _accumulate_predictions_from_multiple_gpus(predictions_per_gpu):
    ...
    if len(image_ids) != image_ids[-1] + 1:
        logger = logging.getLogger("patrans_core.inference")
        logger.warning(
...
def inference(
    ...
    total_timer.tic()
    #predictions, vis = compute_on_dataset(model, dataset, data_loader, device, bbox_aug, cfg.MODEL.VID.METHOD, inference_timer)
    predictions = compute_on_dataset(model, dataset, data_loader, device, bbox_aug, cfg.MODEL.VID.METHOD, inference_timer)
    #print("vis", vis[0].shape)
