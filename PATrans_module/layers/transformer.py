...
import os
import time
from PATrans_module.layers.basic import DropPath, GroupNorm1D, GNActDWConv2d, seq_to_2d
from PATrans_module.layers.attention import GlobalPatternAware,  LocalPatternAware, MultiheadAttention
...
class GlobalLocalPATransBlock(nn.Module):
    ...
    def forward(self,
        ...
        if cross_att_pos:
            ...
            support_K = self.with_pos_embed(support_K, self.pos_generator(support_K))
            support_V = self.with_pos_embed(support_V, self.pos_generator(support_V))
        tgt3 = self.local_channelaware_attn(local_Q, support_K, support_V)[0]
